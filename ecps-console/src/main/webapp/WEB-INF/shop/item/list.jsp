...
<head>
	...
	<script type="text/javascript" src="${path}/ecps/console/res/item/list.js"></script>

</head>
<body id="main">
<form id="showForm" action="${path}/item/listShowModify.do" method="get">
	<input type="hidden" name="itemId" id="itemId" value="" />
	<input type="hidden" name="showStatus" id="showStatus1" value="" />
...
<div class="frameR"><div class="content">
	...
	<h2 class="h2_ch"><span id="tabs" class="l">
        ...
        <a id="label6" href="${path}/item/toList.do"   title="全部实体商品" class="nor">全部</a>
        <a id="label4" href="${path}/item/toList.do?showStatus=1&auditStatus=1"  title="未上架实体商品" class="nor">未上架</a>
        <a id="label5" href="${path}/item/toList.do?showStatus=0&auditStatus=1" title="已上架实体商品" class="nor">已上架</a>
    </span></h2>

	<form id="form1" name="form1" action="${path}/item/toList.do" method="post">
			...
			<c:forEach var="item" items="${page.list}">
				<tr>
					...
					<td>
						...
						<span class="is" <c:if test="${item.isNew == 1}"/> ></span>

					</td>
					<td>
						<span class="not" <c:if test="${item.isGood == 0}"/> ></span>
					</td>
					<td>
						<span class="not" <c:if test="${item.isHot == 0}"/> ></span>
					</td>
					<td>
					...
					<td>
						...
						<c:if test="${item.showStatus == 0 }">
							<a href="javascript:void(0);" onclick="isFaBu(${item.itemId})" >发布</a>   <!-- 静态化 -->
        ...
        <span class="l inb_a">
            ...
            <input type="hidden" value="${page.totalPage}" id="totalPage" name="totalPage" />共<var id="pagePiece" class="orange">${page.totalPage}</var>条<var id="pageTotal">${page.pageNo}/${page.totalPage}</var>
