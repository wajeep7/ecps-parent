...
<head>
    ...
    <script type="text/javascript" src="${path}/ecps/console/res/item/addItem.js"></script>
</head>
<body id="main">
...
<div class="frameR"><div class="content">
    ...
    <form action="${path}/item/addItem.do" name="myForm" id="myForm" method="post">
        <input type="hidden" name="lastFilePath" id="lastFilePath" value="" />
	...
	<a href="javascript:void(0);" ref="#tab_4" title="商品规格" class="nor">商品规格</a>
        ...
        <%--增加规格为了避免互斥。。。拿到button 增加规格 进行互斥操作radipspec1 让每一层增加规格的ID +1 --%>
                        ...
                        <tr>
                            ...
                            <td>
                                <c:if test="${spec.inputType == 2}">
                                    <c:forEach var="val" items="${spec.selectValues}">
                                            <input type="radio" name="${spec.featureId}radipspec1" value="${val }"/>${val }
                                    </c:forEach>
                                </c:if>
                            ...
                            <tr>
                                ...
                                <td width="12%" class="nwp">
                                    ...
                                    <input reg1="^[0-9]{1,7}\.{0,1}[0-9]{0,2}$" desc="保留2位小数，最多允许9位有效数字"
                                           type="text" id="skuPrice" name="skuPrice1"class="text20" size="5" onblur="changePri(this)"/>
                                </td>

                                <td width="12%" class="nwp">
                                    <input type="text" id="marketPrice" name="marketPrice1" class="text20" reg1="^[0-9]{0,7}\.{0,1}[0-9]{0,2}$"
                                           desc="保留2位小数，最多允许9位有效数字" size="5" onblur="changePri(this)"/>
                                </td>

                                <td width="12%" class="nwp">
                                    <samp class="red">*</samp>
                                    <input reg1="^(0|[1-9][0-9]{0,4})$" desc="5个字符以内非负整数" type="text" id="stockInventory"
                                           name="stockInventory1" class="text20" size="5" />
                                </td>

                                <td width="12%" class="nwp">
                                    <input reg1="^(.{0,0}|0|[1-9][0-9]{0,4})$" desc="请输入5个字符以内非负整数或为空 " type="text"
                                           id="skuUpperLimit" name="skuUpperLimit1" class="text20" size="5"/>
                                </td>

                                <td width="12%"  class="nwp">
                                    <input type="text" id="sku" name="sku1" class="text20" reg1="^[a-zA-Z0-9_\u4e00-\u9fa5]{0,20}$"
                                           desc="20个字符以内" size="5"/>
                                </td>

                                <td width="12%" class="nwp" style='display:none;' >
                                    <input reg1="^[a-zA-Z0-9_\u4e00-\u9fa5]{0,20}$" desc="20个字符以内" type="text"
                                           id="location" name="location1" class="text20" size="5" />
                                </td>
                                <td>
